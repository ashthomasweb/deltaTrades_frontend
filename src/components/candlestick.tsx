...
const Candlestick: React.FC<CandleStickProps> = (props: CandleStickProps) => {
  ...
  useEffect(() => {
    if (props.messages.length > 1) {
      ...
      const dataset = Object.values(props.messages[1]?.data["Time Series (1min)"])
  ...
  return (
    <div
      style={{ width: 1000, height: 1000, background: "whitesmoke", padding: 20, borderRadius: 14 }}
    >
      {props.messages.length > 1 ? <ReactECharts option={options} style={styles} /> : null}
    </div>
  )
}
