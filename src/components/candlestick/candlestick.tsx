...
const Candlestick: React.FC<CandleStickProps> = (props: CandleStickProps) => {
  ...
  useEffect(() => {
    ...
    if (props.messages.length > 1 && Object.keys(props.messages[props.messages.length - 1]).includes('data')) {
      console.log('has data')
      const chartData = dataAdapter(props.messages[props.messages.length - 1].data, props.requestParams?.interval as string)
  ...
  return (
    <div className="candlestick-container">
      ...
      {
        options ? (
          <ReactECharts ref={chartRef} notMerge={true} lazyUpdate={false} option={options} style={styles} />
        ) : null
      }
    </div>
  )
